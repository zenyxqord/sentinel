...
import sys
import os
from zenyx_config import ZenyxQordConfig

default_sentinel_config = os.path.normpath(
    os.path.join(os.path.dirname(__file__), "../sentinel.conf")
)
sentinel_config_file = os.environ.get("SENTINEL_CONFIG", default_sentinel_config)
sentinel_cfg = ZenyxQordConfig.tokenize(sentinel_config_file)
sentinel_version = "1.7.3"


def get_zenyx_conf():
    if sys.platform == "win32":
        zenyx_conf = os.path.join(os.getenv("APPDATA"), "ZenyxQord/zenyx.conf")
    else:
        home = os.environ.get("HOME")

        zenyx_conf = os.path.join(home, ".zenyx/zenyx.conf")
        if sys.platform == "darwin":
            zenyx_conf = os.path.join(
                home, "Library/Application Support/ZenyxQord/zenyx.conf"
            )

    zenyx_conf = sentinel_cfg.get("zenyx_conf", zenyx_conf)
