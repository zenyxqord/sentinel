...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, zenyxd):
        golist = zenyxd.rpc_command("gobject", "list")
        ...
        for item in golist.values():
            try:
                (go, subobj) = self.import_gobject_from_zenyxd(zenyxd, item)
            except Exception as e:
    ...
    @classmethod
    def import_gobject_from_zenyxd(self, zenyxd, rec):
        import decimal
        import zenyxlib
        import binascii
        import gobject_json
        ...
        # exclude any invalid model data from zenyxd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from zenyxd, with every run
        ...
        # get/create, then sync payment amounts, etc. from zenyxd - ZenyxQordd is the master
        try:
            newdikt = subdikt.copy()
            newdikt["object_hash"] = object_hash
            if subclass(**newdikt).is_valid(zenyxd) is False:
                govobj.vote_delete(zenyxd)
                return (govobj, None)
            ...
            printdbg("Got invalid object from zenyxd! %s" % e)
            govobj.vote_delete(zenyxd)
            return (govobj, None)
    ...
    def vote_delete(self, zenyxd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(zenyxd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, zenyxd, signal, outcome):
        import zenyxlib
        ...
        output = zenyxd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = zenyxlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(zenyxd, signal)

    def sync_network_vote(self, zenyxd, signal):
        printdbg(
            "\tSyncing network vote for object %s with signal %s"
            % (self.object_hash, signal.name)
        )
        vote_info = zenyxd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (zenyxlib.blocks_to_seconds(superblockcycle) / 2) + SUPERBLOCK_FUDGE_WINDOW
        )
...
class Superblock(BaseModel, GovernanceClass):
    ...
    def is_valid(self, zenyxd=None):
        ...
        network = "mainnet"
        if zenyxd is not None:
            network = zenyxd.network()

        # it's a string from the DB...
        addresses = self.payment_addresses.split("|")
        for addr in addresses:
            if not zenyxlib.is_valid_zenyx_address(addr, network):
    ...
    def hash(self):
        ...
        return zenyxlib.hashit(self.serialise())
