...
class ZenyxQordDaemon:
    def __init__(self, **kwargs):
        ...
        # memoize calls to some zenyxd methods
    ...
    def from_zenyx_conf(self, zenyx_dot_conf):
        from lib.zenyx_config import ZenyxQordConfig

        config_text = ZenyxQordConfig.slurp_config_file(zenyx_dot_conf)
        creds = ZenyxQordConfig.get_rpc_creds(config_text)
    ...
    def network(self):
        # from zenyx/src/chainparamsbase.cpp
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = zenyxlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = zenyxlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_outpoint = self.get_current_masternode_outpoint()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = zenyxlib.blocks_to_seconds(diff)
