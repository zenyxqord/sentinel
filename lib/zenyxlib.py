...
def is_valid_zenyx_address(address, network="mainnet"):
    ...
    p2pkh_version = 75 if network == "testnet" else 25
    p2sh_version = 76 if network == "testnet" else 26
    ...
    try:
        ...
        # rescue from exception, not a valid Zenyx address
        return False
...
def did_we_vote(output):
    ...
    try:
        detail = output.get("detail").get("zenyx.conf")
