...
def has_zenyx_conf():
    ...
    valid_zenyx_conf = False

    # ensure zenyx_conf exists & readable
    #
    # if not, print a message stating that Zenyx Core must be installed and
    # configured, including JSONRPC access in zenyx.conf
    try:
        f = io.open(config.zenyx_conf)
        valid_zenyx_conf = True
    except IOError as e:
        print(e)

    return valid_zenyx_conf
...
def main():
    ...
    if not has_required_env_vars() and not has_zenyx_conf():
        print(
            "ZenyxQord must be installed and configured, including JSONRPC access in zenyx.conf"
        )
        sys.exit(1)

    # deprecation warning
    if not has_required_env_vars() and has_zenyx_conf():
        print(
            "deprecation warning: JSONRPC credentials should now be set using environment variables. Using zenyx.conf will be deprecated in the near future."
        )
