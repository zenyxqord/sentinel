...
def valid_zenyx_address(network="mainnet"):
    return (
...
def invalid_zenyx_address(network="mainnet"):
    return (
...
def test_valid_zenyx_address():
    from zenyxlib import is_valid_zenyx_address

    main = valid_zenyx_address()
    test = valid_zenyx_address("testnet")

    assert is_valid_zenyx_address(main) is True
    assert is_valid_zenyx_address(main, "mainnet") is True
    assert is_valid_zenyx_address(main, "testnet") is False

    assert is_valid_zenyx_address(test) is False
    assert is_valid_zenyx_address(test, "mainnet") is False
    assert is_valid_zenyx_address(test, "testnet") is True


def test_invalid_zenyx_address():
    from zenyxlib import is_valid_zenyx_address

    main = invalid_zenyx_address()
    test = invalid_zenyx_address("testnet")

    assert is_valid_zenyx_address(main) is False
    assert is_valid_zenyx_address(main, "mainnet") is False
    assert is_valid_zenyx_address(main, "testnet") is False

    assert is_valid_zenyx_address(test) is False
    assert is_valid_zenyx_address(test, "mainnet") is False
    assert is_valid_zenyx_address(test, "testnet") is False
...
def test_parse_masternode_status_outpoint():
    from zenyxlib import parse_masternode_status_outpoint
...
def test_hash_function():
    ...
    hex_hash = "%x" % zenyxlib.hashit(sb_data_hex)
...
def test_blocks_to_seconds():
    ...
    assert Decimal(zenyxlib.blocks_to_seconds(0)) == Decimal(0.0)
    assert Decimal(zenyxlib.blocks_to_seconds(2)).quantize(precision) == Decimal(
        314.4
    ).quantize(precision)
    assert int(zenyxlib.blocks_to_seconds(16616)) == 2612035
...
def test_parse_raw_votes():
    ...
    votes = zenyxlib.parse_raw_votes(sample_raw_votes)
    ...
    votes = zenyxlib.parse_raw_votes(sample_raw_votes_weights)
    # sort vote dicts to ensure ordering is the same as the expected output
    # below
    votes = sorted(votes, key=lambda x: x["mn_collateral_outpoint"])
    assert votes == expected
